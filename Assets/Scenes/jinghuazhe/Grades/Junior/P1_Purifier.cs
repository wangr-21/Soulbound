...
public class P1_Purifier : MonoBehaviour
{
    ...
    private void FindCorrectAnimationParameter()
    {
        if (animator == null)
        {
            Debug.LogWarning("P1: 没有Animator组件");
            return;
        }
        ...
        foreach (string name in possibleNames)
        {
            foreach (var param in animator.parameters)
            {
                if (param.name == name && param.type == AnimatorControllerParameterType.Bool)
                {
                    ...
                    Debug.Log($"P1: 使用动画参数 '{walkParameterName}'");
                    return;
                }
            }
        }
        ...
        foreach (var param in animator.parameters)
        {
            if (param.type == AnimatorControllerParameterType.Bool)
            {
                ...
                Debug.LogWarning($"P1: 使用找到的备用参数 '{walkParameterName}'");
                return;
            }
        }

        Debug.LogWarning("P1: 没有找到可用的行走动画参数");
    }
    ...
    private void InitializePatrol()
    {
        ...
        {
            Debug.LogError("P1: 代理不在NavMesh上！需要重新放置对象或重新烘焙NavMesh");
            return;
        }
        ...
        {
            Debug.Log($"P1: 开始巡逻，共有 {P1_patrolPoints.Length} 个巡逻点");
        }
        else
        {
            Debug.LogError("P1: 无法设置初始巡逻目标！");
        }
    }
    ...
    private void HandleMovingStateEnhanced()
    {
        ...
        {
            Debug.LogWarning("P1: 路径无效，重新计算路径");
            if (!SetPatrolTargetEnhanced(currentPatrolIndex))
            {
                // 如果当前点不可达，尝试下一个点
        ...
        if (agent.remainingDistance > pointArrivalDistance &&
            ...
            !agent.pathPending)
        {
            Debug.LogWarning($"P1: 可能卡住，重新路径计算。剩余距离: {agent.remainingDistance}");
        ...
        {
            Debug.Log($"P1: 到达巡逻点 {currentPatrolIndex}，开始等待");
    ...
    private void HandleWaitingState()
    {
        ...
        if (waitCounter <= 0f)
        {
            ...
            if (SetPatrolTargetEnhanced(currentPatrolIndex))
            {
                Debug.Log($"P1: 等待结束，前往巡逻点 {currentPatrolIndex}");
            }
        }
    }
    ...
    private bool SetPatrolTargetEnhanced(int index)
    {
        ...
        if (NavMesh.SamplePosition(targetPosition, out NavMeshHit hit, 2.0f, NavMesh.AllAreas))
        {
            ...
            Debug.Log($"P1: 找到有效NavMesh位置 {targetPosition}");
        }
        else
        {
            Debug.LogError($"P1: 巡逻点 {index} 不在NavMesh上！位置: {targetPosition}");
            return false;
        }
        ...
        if (agent.CalculatePath(targetPosition, path))
        {
            if (path.status == NavMeshPathStatus.PathComplete)
            {
                ...
                Debug.Log($"P1: 成功设置巡逻目标 {index} - {targetPosition}");
                return true;
            }
            else
            {
                Debug.LogWarning($"P1: 路径不可达 (状态: {path.status})，尝试寻找最近可达点");

                // 尝试寻找最近的可达点
                if (NavMesh.FindClosestEdge(targetPosition, out NavMeshHit edgeHit, NavMesh.AllAreas))
                {
                    agent.SetDestination(edgeHit.position);
                    Debug.Log($"P1: 使用最近可达点 {edgeHit.position}");
                    return true;
                }
            }
        }

        Debug.LogError($"P1: 无法计算到巡逻点 {index} 的路径");
        return false;
    }
    ...
    private bool IsPatrolValid(int index)
    {
        ...
        if (index < 0 || index >= P1_patrolPoints.Length)
        {
            Debug.LogError($"P1: 巡逻点索引 {index} 超出范围");
            return false;
        }

        if (P1_patrolPoints[index] == null)
        {
            Debug.LogError($"P1: 巡逻点 {index} 为null");
            return false;
        }

        if (agent == null)
        {
            Debug.LogError("P1: NavMeshAgent 为null");
            return false;
        }
    ...
    private void FixRotationIssue()
    {
        ...
        if (agent.hasPath && agent.remainingDistance > pointArrivalDistance)
        {
            ...
            {
                Debug.LogWarning("P1: 检测到可能卡在旋转上，尝试修复");
    ...
    private void DebugDisplay()
    {
        if (Time.frameCount % 120 == 0) // 每2秒输出一次
        {
            ...
            Debug.Log($"P1状态: {state} | 移动: {moveState} | 目标点: {currentPatrolIndex} | {pathInfo} | 剩余距离: {agent.remainingDistance:F1}");
        }
    }
    ...
    private void OnDrawGizmos()
    {
        ...
        // 绘制当前目标点
        if (P1_patrolPoints != null && currentPatrolIndex < P1_patrolPoints.Length && P1_patrolPoints[currentPatrolIndex] != null)
        {
    ...
    private void ShowNavMeshInfo()
    {
        ...
        if (P1_patrolPoints != null)
        {
            for (int i = 0; i < P1_patrolPoints.Length; i++)
            {
                if (P1_patrolPoints[i] != null)
                {
                    bool onNavMesh = NavMesh.SamplePosition(P1_patrolPoints[i].position, out hit, 1f, NavMesh.AllAreas);
                    Debug.Log($"巡逻点 {i}: {P1_patrolPoints[i].position} - 在NavMesh上: {onNavMesh}");
                }
            }
        }
    }
    ...
    private void DiagnosePatrolIssue()
    {
        ...
        if (P1_patrolPoints != null && currentPatrolIndex < P1_patrolPoints.Length)
        {
            Vector3 targetPos = P1_patrolPoints[currentPatrolIndex].position;
