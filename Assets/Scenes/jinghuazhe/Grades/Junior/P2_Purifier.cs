...
public class P2_Purifier : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            SetNextPatrolTarget();
            Debug.Log($"P2 脚本启动，开始巡逻。视野半径: {visionRadius}, 逃脱距离: {escapeDistance}");
        }
        else
        {
    ...
    void Update()
    {
        ...
        if (player != null)
        {
            ...
            {
                Debug.Log($"P2-距离: {currentDistance:F1}, 视野: {visionRadius}, 逃脱: {escapeDistance}, 检测: {isPlayerInSight}");
            }
        }
    ...
    private void CheckPlayerInSight()
    {
        ...
        {
            Debug.Log($"P2: 成功检测到玩家！距离: {distance:F1}, 角度: {angle:F1}");
        }
    ...
    private void CheckPlayerEscape()
    {
        ...
        {
            Debug.Log($"P2: 玩家逃脱到安全距离 ({distanceToPlayer:F1} > {escapeDistance})，恢复巡逻");
            isPlayerInSight = false;
    ...
    private void HandleMovingState()
    {
        ...
        if (!agent.pathPending && agent.remainingDistance <= pointArrivalDistance)
        {
            ...
            agent.isStopped = true;
            Debug.Log($"P2: 到达巡逻点 {currentPatrolIndex}，等待 {waitTimeAtPoint}秒");
        }
    }
    ...
    private void ShowCurrentSettings()
    {
        ...
        Debug.Log($"逃脱距离: {escapeDistance}");
        Debug.Log($"巡逻点数量: {P2_patrolPoints?.Length ?? 0}");
