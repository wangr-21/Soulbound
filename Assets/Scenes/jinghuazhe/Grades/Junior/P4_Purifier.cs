...
public class P4_Purifier : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            Debug.LogWarning("P4 未找到标签为Player的物体！");
        }
        ...
        {
            SetNextPatrolTarget();
            Debug.Log($"P4 脚本启动，开始巡逻。视野半径: {visionRadius}, 逃脱距离: {escapeDistance}");
        }
        else
        {
            Debug.LogWarning("P4 未设置巡逻点！");
        }
    }
    ...
    void Update()
    {
        ...
        if (player != null)
        {
            ...
            {
                Debug.Log($"P4-距离: {currentDistance:F1}, 视野: {visionRadius}, 逃脱: {escapeDistance}, 检测: {isPlayerInSight}");
            }
        }
    ...
    private void CheckPlayerInSight()
    {
        ...
        {
            Debug.Log($"P4: 成功检测到玩家！距离: {distance:F1}, 角度: {angle:F1}");
        }
    ...
    private void CheckPlayerEscape()
    {
        ...
        {
            Debug.Log($"P4: 玩家逃脱到安全距离 ({distanceToPlayer:F1} > {escapeDistance})，恢复巡逻");
            isPlayerInSight = false;
    ...
    private void HandleMovingState()
    {
        ...
        if (!agent.pathPending && agent.remainingDistance <= pointArrivalDistance)
        {
            ...
            agent.isStopped = true;
            Debug.Log($"P4: 到达巡逻点 {currentPatrolIndex}，等待 {waitTimeAtPoint}秒");
        }
    }
    ...
    {
        if (P4_patrolPoints.Length > 0 && P4_patrolPoints[currentPatrolIndex] != null && agent != null)
        {
            agent.SetDestination(P4_patrolPoints[currentPatrolIndex].position);
        }
    }
    ...
    private void ShowCurrentSettings()
    {
        ...
        Debug.Log($"逃脱距离: {escapeDistance}");
        Debug.Log($"巡逻点数量: {P4_patrolPoints?.Length ?? 0}");
