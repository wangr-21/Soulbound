...
public class P8_Purifier : MonoBehaviour
{
    ...
    private void InitializePurifier()
    {
        ...
        Debug.Log($"P8净化者初始化完成 - 位置: {transform.position}");
    }
    ...
    private void FindPlayer()
    {
        ...
        if (playerObj != null)
        {
            ...
            Debug.Log($"P8: 找到玩家: {player.name}");
        }
        else
        {
            Debug.LogError("P8: 未找到玩家对象（需添加Player标签）！");
        }
    }
    ...
    private void InitializeRenderer()
    {
        ...
        if (purifierRenderer != null)
        {
            ...
            Debug.Log($"P8: 渲染器初始化完成 - 材质: {originalMaterial.name}");
        }
        else
        {
            Debug.LogWarning("P8: 找不到Renderer组件，颜色闪烁效果将不可用");
        }
    }
    ...
    private void InitializeCollider()
    {
        ...
        if (purifierCollider == null)
        {
            ...
            Debug.Log($"P8: 已添加CapsuleCollider - 半径: {colliderRadius}, 高度: {colliderHeight}");
        }
        else
        {
            // 调整现有碰撞体大小
        ...
        if (rb == null)
        {
            ...
            Debug.Log("P8: 已添加Rigidbody组件（运动学，冻结旋转）");
        }
        else
        {
            // 确保现有的Rigidbody设置正确
    ...
    private void InitializeParticleSystem()
    {
        ...
        if (alertParticleSystem != null)
        {
            ...
            Debug.Log("P8: 警报粒子系统初始化完成");
        }
        else
        {
            Debug.LogWarning("P8: 未找到警报粒子系统");
        }
    }
    ...
    private void FindCorrectAnimationParameter()
    {
        if (animator == null)
        {
            Debug.LogWarning("P8: 未找到Animator组件");
            return;
        }
        ...
        foreach (string name in possibleWalkNames)
        {
            foreach (var param in animator.parameters)
            {
                if (param.name == name && param.type == AnimatorControllerParameterType.Bool)
                {
                    ...
                    Debug.Log($"P8: 找到行走参数: {walkParameterName}");
                    break;
                }
            }
        }
        ...
        foreach (string name in possibleAttackNames)
        {
            foreach (var param in animator.parameters)
            {
                if (param.name == name && param.type == AnimatorControllerParameterType.Bool)
                {
                    ...
                    Debug.Log($"P8: 找到攻击参数: {attackParameterName}");
                    break;
                }
            }
        }
    ...
    private void InitializePatrol()
    {
        ...
        {
            Debug.LogError("P8: 代理不在NavMesh上！");
            return;
        }
        ...
        Debug.Log($"P8: 巡逻初始化完成 - 巡逻点数量: {P8_patrolPoints.Length}");
    }
    ...
    private void ApplyDamageToTarget()
    {
        ...
        if (currentTarget.CompareTag("Player"))
        {
            ...
            if (playerSoul != null)
            {
                ...
                Debug.Log($"P8净化者攻击玩家灵魂，造成{attackDamage}点伤害");
            }
        }
        else
        {
            ...
            if (possessable != null && possessable is MonoBehaviour)
            {
                ...
                if (deer != null)
                {
                    ...
                    Debug.Log($"P8净化者攻击鹿，造成{finalDamage}点伤害 (基础{attackDamage} × 倍率{deerDamageMultiplier})");
                    return;
                }
                ...
                if (sheep != null)
                {
                    ...
                    Debug.Log($"P8净化者攻击绵羊，造成{finalDamage}点伤害 (基础{attackDamage} × 倍率{sheepDamageMultiplier})");
                    return;
                }
                ...
                if (fox != null)
                {
                    ...
                    Debug.Log($"P8净化者攻击狐狸，造成{finalDamage}点伤害 (基础{attackDamage} × 倍率{foxDamageMultiplier})");
                    return;
                }
                ...
                if (bird != null)
                {
                    ...
                    Debug.Log($"P8净化者攻击鸟，造成{finalDamage}点伤害 (基础{attackDamage} × 倍率{birdDamageMultiplier})");
                    return;
                }
    ...
    {
        if (P8_patrolPoints == null || P8_patrolPoints.Length == 0) return false;
        if (index < 0 || index >= P8_patrolPoints.Length) return false;
        if (P8_patrolPoints[index] == null) return false;
    ...
    private void HandleCollision(GameObject collidedObject)
    {
        ...
        if (IsPlayerOrPossessedObject(collidedObject))
        {
            ...
            Debug.Log($"P8净化者与{collidedObject.name}碰撞，造成伤害");
        }
    }
    ...
    private void OnDrawGizmos()
    {
        ...
        {
            // 当前目标点
            if (currentPatrolIndex < P8_patrolPoints.Length && P8_patrolPoints[currentPatrolIndex] != null)
            {
                Gizmos.color = Color.yellow;
            ...
            for (int i = 0; i < P8_patrolPoints.Length; i++)
            {
                if (P8_patrolPoints[i] != null)
                {
                    ...
                    Gizmos.DrawSphere(P8_patrolPoints[i].position, 0.2f);
                }
            }
        }
    ...
    private void SimulatePlayerCollision()
    {
        ...
        {
            Debug.LogWarning("P8: 无法找到玩家进行模拟碰撞");
        }
    }
