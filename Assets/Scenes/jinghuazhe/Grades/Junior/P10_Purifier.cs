...
public class P10_Purifier : MonoBehaviour
{
    [Header("=== P10专属设置 ===")]
    public Transform[] P10_patrolPoints;
    ...
    private void InitializePurifier()
    {
        ...
        if (agent == null)
        {
            Debug.LogError("P10: 找不到NavMeshAgent组件！");
            return;
        }
    ...
    private void FindPlayer()
    {
        ...
        if (playerObj != null)
        {
            ...
            Debug.Log("P10: 找到玩家对象");
        }
        else
        {
            Debug.LogWarning("P10: 未找到玩家对象");
        }
    }
    ...
    private void FindCorrectAnimationParameter()
    {
        if (animator == null)
        {
            Debug.LogWarning("P10: 没有Animator组件");
            return;
        }
        ...
        foreach (string name in possibleNames)
        {
            foreach (var param in animator.parameters)
            {
                if (param.name == name && param.type == AnimatorControllerParameterType.Bool)
                {
                    ...
                    Debug.Log($"P10: 使用动画参数 '{walkParameterName}'");
                    return;
                }
            }
        }
        ...
        foreach (var param in animator.parameters)
        {
            if (param.type == AnimatorControllerParameterType.Bool)
            {
                ...
                Debug.LogWarning($"P10: 使用找到的备用参数 '{walkParameterName}'");
                return;
            }
        }

        Debug.LogWarning("P10: 没有找到可用的行走动画参数");
    }
    ...
    private void InitializePatrol()
    {
        ...
        {
            Debug.LogError("P10: 代理不在NavMesh上！需要重新放置对象或重新烘焙NavMesh");
            return;
        }

        if (P10_patrolPoints == null || P10_patrolPoints.Length == 0)
        {
            Debug.LogError("P10: 巡逻点数组为空！");
            return;
        }

        // 验证巡逻点
        for (int i = 0; i < P10_patrolPoints.Length; i++)
        {
            if (P10_patrolPoints[i] == null)
            {
                Debug.LogError($"P10: 巡逻点 {i} 为null！");
                return;
            }
        }
        ...
        {
            Debug.Log($"P10: 开始巡逻，共有 {P10_patrolPoints.Length} 个巡逻点");
        }
        else
        {
            Debug.LogError("P10: 无法设置初始巡逻目标！");
        }
    }
    ...
    {
        if (P10_patrolPoints.Length == 0) return;

        // 检查是否没有路径或路径无效
        if (!agent.hasPath || agent.pathStatus == NavMeshPathStatus.PathInvalid)
        {
            Debug.LogWarning("P10: 路径无效，重新计算路径");
            if (!SetPatrolTargetEnhanced(currentPatrolIndex))
            {
                // 如果当前点不可达，尝试下一个点
                currentPatrolIndex = (currentPatrolIndex + 1) % P10_patrolPoints.Length;
        ...
        if (agent.remainingDistance > pointArrivalDistance &&
            ...
            !agent.pathPending)
        {
            Debug.LogWarning($"P10: 可能卡住，重新路径计算。剩余距离: {agent.remainingDistance}");
        ...
        {
            Debug.Log($"P10: 到达巡逻点 {currentPatrolIndex}，开始等待");
    ...
    private void HandleWaitingState()
    {
        ...
        if (waitCounter <= 0f)
        {
            ...
            if (SetPatrolTargetEnhanced(currentPatrolIndex))
            {
                Debug.Log($"P10: 等待结束，前往巡逻点 {currentPatrolIndex}");
            }
        }
    }
    ...
    private bool SetPatrolTargetEnhanced(int index)
    {
        ...
        Vector3 targetPosition = P10_patrolPoints[index].position;

        // 检查目标点是否在NavMesh上
        if (NavMesh.SamplePosition(targetPosition, out NavMeshHit hit, 2.0f, NavMesh.AllAreas))
        {
            targetPosition = hit.position;
            Debug.Log($"P10: 找到有效NavMesh位置 {targetPosition}");
        }
        else
        {
            Debug.LogError($"P10: 巡逻点 {index} 不在NavMesh上！位置: {targetPosition}");
            return false;
        }
        ...
        if (agent.CalculatePath(targetPosition, path))
        {
            if (path.status == NavMeshPathStatus.PathComplete)
            {
                ...
                Debug.Log($"P10: 成功设置巡逻目标 {index} - {targetPosition}");
                return true;
            }
            else
            {
                Debug.LogWarning($"P10: 路径不可达 (状态: {path.status})，尝试寻找最近可达点");

                // 尝试寻找最近的可达点
                if (NavMesh.FindClosestEdge(targetPosition, out NavMeshHit edgeHit, NavMesh.AllAreas))
                {
                    agent.SetDestination(edgeHit.position);
                    Debug.Log($"P10: 使用最近可达点 {edgeHit.position}");
                    return true;
                }
            }
        }

        Debug.LogError($"P10: 无法计算到巡逻点 {index} 的路径");
        return false;
    }
    ...
    private bool IsPatrolValid(int index)
    {
        ...
        if (index < 0 || index >= P10_patrolPoints.Length)
        {
            Debug.LogError($"P10: 巡逻点索引 {index} 超出范围");
            return false;
        }

        if (P10_patrolPoints[index] == null)
        {
            Debug.LogError($"P10: 巡逻点 {index} 为null");
            return false;
        }

        if (agent == null)
        {
            Debug.LogError("P10: NavMeshAgent 为null");
            return false;
        }
    ...
    private void FixRotationIssue()
    {
        ...
        if (agent.hasPath && agent.remainingDistance > pointArrivalDistance)
        {
            ...
            {
                Debug.LogWarning("P10: 检测到可能卡在旋转上，尝试修复");
    ...
    private void DebugDisplay()
    {
        if (Time.frameCount % 120 == 0) // 每2秒输出一次
        {
            ...
            Debug.Log($"P10状态: {state} | 移动: {moveState} | 目标点: {currentPatrolIndex} | {pathInfo} | 剩余距离: {agent.remainingDistance:F1}");
        }
    }
    ...
    private void OnDrawGizmos()
    {
        ...
        // 绘制当前目标点
        if (P10_patrolPoints != null && currentPatrolIndex < P10_patrolPoints.Length && P10_patrolPoints[currentPatrolIndex] != null)
        {
            Gizmos.color = Color.yellow;
            Gizmos.DrawWireSphere(P10_patrolPoints[currentPatrolIndex].position, 0.5f);
        }
    }
    ...
    private void ShowNavMeshInfo()
    {
        ...
        if (P10_patrolPoints != null)
        {
            for (int i = 0; i < P10_patrolPoints.Length; i++)
            {
                if (P10_patrolPoints[i] != null)
                {
                    bool onNavMesh = NavMesh.SamplePosition(P10_patrolPoints[i].position, out hit, 1f, NavMesh.AllAreas);
                    Debug.Log($"巡逻点 {i}: {P10_patrolPoints[i].position} - 在NavMesh上: {onNavMesh}");
                }
            }
        }
    }
    ...
    private void DiagnosePatrolIssue()
    {
        ...
        if (P10_patrolPoints != null && currentPatrolIndex < P10_patrolPoints.Length)
        {
            Vector3 targetPos = P10_patrolPoints[currentPatrolIndex].position;
